...
//  }
//
//  protected Pair<CubicCoordinate, Integer> miniMax(ReadonlyReversiModel model,
//                                                   int player, int depth) {
//
//    // get the list of possible moves for each player
//    List<CubicCoordinate> availablePlayer = model.getAvailableMoves(player);
//    List<CubicCoordinate> availableOpponent = model.getAvailableMoves(3 - player);
//
//    //max depth reached
//    if (depth >= this.depth) {
//      // if ME has no moves
...
//      else {
//        ReversiModel possibleGame = model.clone();
//        CubicCoordinate move = this.opponentStrategy.chooseCoord(possibleGame, player);
//        possibleGame.flipCell(move, player);
//        return new Pair<>(move, miniMaxScore(possibleGame, player));
//      }
//    }
...
//    }
//
//    List<CubicCoordinate> available = model.getAvailableMoves(player);
//    //which possible move is best
//    List<Pair<CubicCoordinate, Integer>> coordScorePairs = new ArrayList<>();
//
//    //    System.out.println("Began iterating over moves at depth " + depth);
//    //populate lists(turn into helper)?
//    //    System.out.println(available);
//    for (CubicCoordinate coord : available) {
...
//  }
//
//  private Pair<CubicCoordinate, Integer> getMaxPair(List<Pair<CubicCoordinate,
//          Integer>> coordScorePairs) {
//    if (coordScorePairs.isEmpty()) {
//      throw new RuntimeException("bad");
//    }
//    Pair<CubicCoordinate, Integer> bestPair = coordScorePairs.get(0);
//
//    for (Pair<CubicCoordinate, Integer> pair : coordScorePairs) {
...
//  }
//
//  private Pair<CubicCoordinate, Integer> getMinPair(List<Pair<CubicCoordinate,
//          Integer>> coordScorePairs) {
//    if (coordScorePairs.isEmpty()) {
//      throw new RuntimeException("bad");
//    }
